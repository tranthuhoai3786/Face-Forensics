...
model_training_first.compile(optimizer = 'adam',#ky thuat toi uu hoa adam
                             ...
                             metrics = ['accuracy'])
model_training_first.fit(np.array([x[0] for _,x in enumerate(X_train)]),np.array([y[1] for _,y in enumerate(X_train)]) , epochs =10)#chay 10 lan
model_training_first.save('model_faceforensics_10epochs.keras')
# import time
...
models = models.load_model('model_faceforensics_10epochs.keras')
while (True):
    OK, frame = cam.read()
